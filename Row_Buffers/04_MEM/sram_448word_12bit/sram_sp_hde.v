...
//       
//       Creation Date:  Mon Nov 27 18:09:58 2023
//       Version: 	r11p2
//
...
`endif
  ...
  reg  read_mux_sel0;
  reg [11:0] Q_int;
  reg [11:0] Q_int_delayed;
  reg [11:0] writeEnable;

  reg NOT_CEN, NOT_WEN, NOT_A8, NOT_A7, NOT_A6, NOT_A5, NOT_A4, NOT_A3, NOT_A2, NOT_A1;
  reg NOT_A0, NOT_D11, NOT_D10, NOT_D9, NOT_D8, NOT_D7, NOT_D6, NOT_D5, NOT_D4, NOT_D3;
  reg NOT_D2, NOT_D1, NOT_D0, NOT_EMA2, NOT_EMA1, NOT_EMA0, NOT_EMAW1, NOT_EMAW0, NOT_EMAS;
  reg NOT_TEN, NOT_TCEN, NOT_TWEN, NOT_TA8, NOT_TA7, NOT_TA6, NOT_TA5, NOT_TA4, NOT_TA3;
  reg NOT_TA2, NOT_TA1, NOT_TA0, NOT_TD11, NOT_TD10, NOT_TD9, NOT_TD8, NOT_TD7, NOT_TD6;
  reg NOT_TD5, NOT_TD4, NOT_TD3, NOT_TD2, NOT_TD1, NOT_TD0, NOT_RET1N, NOT_STOV;
 ...
 wire  CLK_;
  ...
  assign `ARM_UD_DP DY_ = RET1N_ ? (TEN_ ? D_ : TD_) : {12{1'bx}};
  assign `ARM_UD_SEQ Q_ = RET1N_ ? (BEN_ ? ((STOV_ ? (Q_int_delayed) : (Q_int))) : TQ_) : {12{1'bx}};
  ...
  begin
	$readmemb(filename, memld);
     if (CEN_ === 1'b1) begin
      ...
      row = mem[row_address];
        writeEnable = {12{1'b1}};
        row_mask =  ( {7'b0000000, writeEnable[11], 7'b0000000, writeEnable[10], 7'b0000000, writeEnable[9],
        ...
        new_data =  ( {7'b0000000, wordtemp[11], 7'b0000000, wordtemp[10], 7'b0000000, wordtemp[9],
	...
	dump_file_desc = $fopen(filename_dump);
     if (CEN_ === 1'b1) begin
      ...
      row = mem[row_address];
        ...
        Q_int = {shifted_readLatch0[22], shifted_readLatch0[20], shifted_readLatch0[18],
  ...
  begin
    if (RET1N_int === 1'bx || RET1N_int === 1'bz) begin
     ...
     === 1'bx) begin
      ...
      Q_int = WEN_int !== 1'b1 ? D_int : {12{1'bx}};
      Q_int_delayed = WEN_int !== 1'b1 ? D_int : {12{1'bx}};
      ...
      if (row_address > 55)
        ...
        row = mem[row_address];
      writeEnable = ~{12{WEN_int}};
      if (WEN_int !== 1'b1) begin
        row_mask =  ( {7'b0000000, writeEnable[11], 7'b0000000, writeEnable[10], 7'b0000000, writeEnable[9],
          7'b0000000, writeEnable[8], 7'b0000000, writeEnable[7], 7'b0000000, writeEnable[6],
          7'b0000000, writeEnable[5], 7'b0000000, writeEnable[4], 7'b0000000, writeEnable[3],
          7'b0000000, writeEnable[2], 7'b0000000, writeEnable[1], 7'b0000000, writeEnable[0]} << mux_address);
        new_data =  ( {7'b0000000, D_int[11], 7'b0000000, D_int[10], 7'b0000000, D_int[9],
...
`endif
 ...
 wire  CLK_;
  ...
  buf B62(EMAW_[0], EMAW[0]);
  buf B63(EMAW_[1], EMAW[1]);
  buf B64(EMAS_, EMAS);
  buf B65(TEN_, TEN);
  buf B66(BEN_, BEN);
  buf B67(TCEN_, TCEN);
  ...
  buf B86(TD_[8], TD[8]);
  buf B87(TD_[9], TD[9]);
  buf B88(TD_[10], TD[10]);
  buf B89(TD_[11], TD[11]);
  buf B90(TQ_[0], TQ[0]);
  buf B91(TQ_[1], TQ[1]);
  buf B92(TQ_[2], TQ[2]);
  buf B93(TQ_[3], TQ[3]);
...
begin
   ...
   `ifdef ARM_NONREPAIRABLE_FAULT
      `pre_pend_path.SMARCHCHKBVCD_LVISION_MBISTPG_ASSEMBLY_UNDER_TEST_INST.MEM0_MEM_INST.u1.add_fault(9'd256,4'd10,2'd1,2'd0);
   `endif
end
   task add_fault;
   //This task injects fault in memory
   //In order to inject fault in redundant column for Bit 0 to 5, column address
   //should have value in range of 4 to 7
   //In order to inject fault in redundant column for Bit 6 to 11, column address
